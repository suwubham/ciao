...
export default function Rdraw(props) {
  ...
  const Sketch = (p5) => {
    p5.setup = () => {
        ...
        saveButton.position(350, p5.height+150);
        // saveButton.fill(200,30,50);
        saveButton.mousePressed(p5.saveArt);
        p5.colorMode(p5.RGB);
    ...
    p5.draw = () => {
     ...
     for (var i = 0; i < p5.width; i += size) {
      ...
      if (n1 < 0.5) {
        p5.fill(p5.floor(n1*100)+p5.colorStart,100,100,100);
          p5.fill(
             props.triangle1.rgb.r,
                ...
                props.triangle1.rgb.b,props.alpha
        );
        p5.triangle(i, j, i + size, j + size, i,j+size);
        p5.fill(p5.floor(100-(n1*100))+p5.colorStart,100,100,props.alpha);
        ...
        p5.fill(p5.floor(100-(n2*100))+p5.colorStart,100,100,props.alpha); 
           p5.fill( props.triangle2.rgb.r,
             props.triangle2.rgb.g,
             props.triangle2.rgb.b,props.alpha
         );
