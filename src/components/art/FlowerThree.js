  ...
  const Sketch = (p5) => {
    ...
    p5.background(
      ...
      for(let phi=0;phi<=360;phi+=0.9){
        let r=(70 *p5.pow(p5.abs(p5.sin(phi*5/2)),1)+225)*theta/60;
        let x=r*p5.cos(phi);
        let y=r*p5.sin(phi);
        //let bumpiness=2*pow(r/100,2)*sin(phi*12);
        //let z=300*pow(Math.E,-0.1*pow(abs(r/100),1.5))*pow(r/100,1/2)-200+bumpiness;
        let z=p5.vShape(300,r/100,0.8,0.15,1.5)-200+p5.bumpiness(1.5,r/100,12,phi);
        p5.vertex(x,y,z);
      }
      }
      a+=30;
      ...
      p5.vShape = (A,r,a,b,c) => {
            return A*p5.pow(Math.E,-b*p5.pow(p5.abs(r),c))*p5.pow(p5.abs(r),a);
      };
